...
/** Separates a comma-separated line, properly escaping commas within
    ...
    \sa G3DLite::stringSplit, G3DLite::TextInput, G3DLite::readWholeFile
*/
...
/** Finds the index of the first '\\' or '/' character, starting at index \a start (if \a start is string::npos, starts at the end of the string).
  \sa G3DLite::findSlash, G3DLite::isSlash
  */
 ...
 */
G3DLite::Array<std::string> stringSplit(
    const std::string&          x,
    char                        splitChar);
...
std::string stringJoin(
    const G3DLite::Array<std::string>&   a,
    char                        joinChar);
