...
public:
    ...
    Color3uint8 max(const Color3uint8 x) const {
        return Color3uint8(G3DLite::max(r, x.r), G3DLite::max(g, x.g), G3DLite::max(b, x.b));
    }

    Color3uint8 min(const Color3uint8 x) const {
        return Color3uint8(G3DLite::min(r, x.r), G3DLite::min(g, x.g), G3DLite::min(b, x.b));
    }
