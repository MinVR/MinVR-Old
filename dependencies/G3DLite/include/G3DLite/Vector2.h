...
public:
    ...
    inline Vector2 clamp(float low, float high) const {
        return Vector2(
            (float)G3DLite::clamp(x, low, high),
            (float)G3DLite::clamp(y, low, high));
    }
...
inline Vector2 Vector2::min(const Vector2 &v) const {
    return Vector2(G3DLite::min(v.x, x), G3DLite::min(v.y, y));
}



inline Vector2 Vector2::max(const Vector2 &v) const {
    return Vector2(G3DLite::max(v.x, x), G3DLite::max(v.y, y));
}
...
inline bool Vector2::isFinite() const {
    return G3DLite::isFinite(x) && G3DLite::isFinite(y);
}
...
struct HashTrait<G3DLite::Vector2> {
    static size_t hashCode(const G3DLite::Vector2& key) {
        return key.hashCode();
    }
};
...
inline G3DLite::Vector2 operator*(int s, const G3DLite::Vector2& v) {
    return v * (float)s;
}
