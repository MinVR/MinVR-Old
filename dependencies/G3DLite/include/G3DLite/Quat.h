...
public:
    ...
    bool fuzzyEq(const Quat& q) {
      return G3DLite::fuzzyEq(x, q.x) && G3DLite::fuzzyEq(y, q.y) && G3DLite::fuzzyEq(z, q.z) && G3DLite::fuzzyEq(w, q.w);
    }
...
inline G3DLite::Quat operator*(double s, const G3DLite::Quat& q) {
    return q * (float)s;
}

inline G3DLite::Quat operator*(float s, const G3DLite::Quat& q) {
    return q * s;
}
...
inline G3DLite::Quat pow(const G3DLite::Quat& q, double x) {
