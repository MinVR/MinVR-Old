...
#ifndef G3DLITE_COLOR4UINT8_H
#define G3DLITE_COLOR4UINT8_H
...
/**
 ...
 G3DLite::GImage as the underlying format.
...
public:
    ...
    Color4uint8 max(const Color4uint8 x) const {
        return Color4uint8(G3DLite::max(r, x.r), G3DLite::max(g, x.g), G3DLite::max(b, x.b), G3DLite::max(a, x.a));
    }

    Color4uint8 min(const Color4uint8 x) const {
        return Color4uint8(G3DLite::min(r, x.r), G3DLite::min(g, x.g), G3DLite::min(b, x.b), G3DLite::min(a, x.a));
    }
