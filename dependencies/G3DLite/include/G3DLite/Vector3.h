...
public:
    ...
    inline float min() const {
        return G3DLite::min(G3DLite::min(x, y), z);
    }

    /** Largest element */
    inline float max() const {
        return G3DLite::max(G3DLite::max(x, y), z);
    }
    ...
    inline Vector3 clamp(const Vector3& low, const Vector3& high) const {
        return Vector3(
            G3DLite::clamp(x, low.x, high.x),
            G3DLite::clamp(y, low.y, high.y),
            G3DLite::clamp(z, low.z, high.z));
    }

    inline Vector3 clamp(float low, float high) const {
        return Vector3(
            G3DLite::clamp(x, low, high),
            G3DLite::clamp(y, low, high),
            G3DLite::clamp(z, low, high));
    }
    ...
    /** \brief Random unit vector, uniformly distributed on the sphere. 
       ...
       Distribution rendered by G3DLite::DirectionHistogram:
    ...
    */
    ...
    inline void getTangents(Vector3& X, Vector3& Y) const {
        debugAssertM(G3DLite::fuzzyEq(length(), 1.0f), 
...
inline G3DLite::Vector3 operator*(float s, const G3DLite::Vector3& v) {
    return v * s;
}

inline G3DLite::Vector3 operator*(double s, const G3DLite::Vector3& v) {
    return v * (float)s;
}

inline G3DLite::Vector3 operator*(int s, const G3DLite::Vector3& v) {
    return v * (float)s;
}
...
inline bool Vector3::fuzzyEq(const Vector3& other) const {
    return G3DLite::fuzzyEq((*this - other).squaredMagnitude(), 0);
}

//----------------------------------------------------------------------------

inline bool Vector3::fuzzyNe(const Vector3& other) const {
    return G3DLite::fuzzyNe((*this - other).squaredMagnitude(), 0);
}

//----------------------------------------------------------------------------

inline bool Vector3::isFinite() const {
    return G3DLite::isFinite(x) && G3DLite::isFinite(y) && G3DLite::isFinite(z);
}
...
inline Vector3 Vector3::min(const Vector3 &v) const {
    return Vector3(G3DLite::min(v.x, x), G3DLite::min(v.y, y), G3DLite::min(v.z, z));
}

//----------------------------------------------------------------------------
inline Vector3 Vector3::max(const Vector3 &v) const {
    return Vector3(G3DLite::max(v.x, x), G3DLite::max(v.y, y), G3DLite::max(v.z, z));
}

//----------------------------------------------------------------------------
inline bool Vector3::isZero() const {
    return G3DLite::fuzzyEq(squaredMagnitude(), 0.0f);
}
...
struct HashTrait<G3DLite::Vector3> {
    static size_t hashCode(const G3DLite::Vector3& key) {
        return key.hashCode();
    }
};


template<> struct PositionTrait<class G3DLite::Vector2> {
    static void getPosition(const G3DLite::Vector2& v, G3DLite::Vector3& p) { p = G3DLite::Vector3(v, 0); }
};
