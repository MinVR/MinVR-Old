/**
  ...
  @edited  2009-04-25
*/
#ifndef G3DLITE_ReferenceCount_h
#define G3DLITE_ReferenceCount_h

#ifdef G3DLITE_USE_SHARED_PTR

	namespace G3DLite {

	#define G3DLite::ReferenceCountedPointer shared_ptr
	#define G3DLite::WeakReferenceCountedPointer weak_ptr
	...
	class Foo : public G3DLite::ReferenceCountedObject {
	public:
		int x;
	};
	...
	typedef G3DLite::ReferenceCountedPointer<Foo> FooRef;
	typedef G3DLite::WeakReferenceCountedPointer<Foo> WeakFooRef;
	typedef G3DLite::ReferenceCountedPointer<Bar> BarRef;
	...
	private:
		...
		void zeroPointer() {
			if (m_pointer != NULL) {
				...
				debugAssert(G3DLite::isValidHeapPointer(m_pointer));
		...
		void setPointer(T* x) {
			if (x != m_pointer) {
				...
				if (x != NULL) {
					debugAssert(G3DLite::isValidHeapPointer(x));
