...
const Vector3& Vector3::inf()      { static const Vector3 v((float)G3DLite::finf(), (float)G3DLite::finf(), (float)G3DLite::finf()); return v; }
const Vector3& Vector3::nan()      { static const Vector3 v((float)G3DLite::fnan(), (float)G3DLite::fnan(), (float)G3DLite::fnan()); return v; }
...
Vector3 Vector3::cosHemiRandom(const Vector3& normal, Random& r) {
    debugAssertM(G3DLite::fuzzyEq(normal.length(), 1.0f), 
...
void Vector3::generateOrthonormalBasis (Vector3& rkU, Vector3& rkV,
    ...
    if ( G3DLite::abs(rkW.x) >= G3DLite::abs(rkW.y)
            && G3DLite::abs(rkW.x) >= G3DLite::abs(rkW.z) ) {
        rkU.x = -rkW.y;
        rkU.y = + rkW.x;
        rkU.z = 0.0;
    } else {
...
std::string Vector3::toString() const {
    return G3DLite::format("(%g, %g, %g)", x, y, z);
}
