...
bool Color3::isFinite() const {
    return G3DLite::isFinite(r) && G3DLite::isFinite(g) && G3DLite::isFinite(b);
}
...
Color3& Color3::operator/= (float fScalar) {
    if (fScalar != 0.0f) {
        ...
        r = (float)G3DLite::finf();
        g = (float)G3DLite::finf();
        b = (float)G3DLite::finf();
    }
...
Color3 Color3::fromHSV(const Vector3& _hsv) {
    ...
    const int i = iMin(5, G3DLite::iFloor(6.0 * _hsv.x));
...
Vector3 Color3::toHSV(const Color3& _rgb) {
	...
	hsv.z = G3DLite::max(G3DLite::max(_rgb.r, _rgb.g), _rgb.b);
	if (G3DLite::fuzzyEq(hsv.z, 0.0f)) {
		return hsv;
	}
	
    const float x =  G3DLite::min(G3DLite::min(_rgb.r, _rgb.g), _rgb.b);
	hsv.y = (hsv.z - x) / hsv.z; 

    if (G3DLite::fuzzyEq(hsv.y, 0.0f)) {
		return hsv;
	}
...
Color3 Color3::jetColorMap(const float& val) {
	...
	jet.r = G3DLite::min(4.0f * val - 1.5f,-4.0f * val + 4.5f) ;
	jet.g = G3DLite::min(4.0f * val - 0.5f,-4.0f * val + 3.5f) ;
	jet.b = G3DLite::min(4.0f * val + 0.5f,-4.0f * val + 2.5f) ;


	jet.r = G3DLite::clamp(jet.r, 0.0f, 1.0f);
	jet.g = G3DLite::clamp(jet.g, 0.0f, 1.0f);
	jet.b = G3DLite::clamp(jet.b, 0.0f, 1.0f);
...
std::string Color3::toString() const {
    return G3DLite::format("(%g, %g, %g)", r, g, b);
}
