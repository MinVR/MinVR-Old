...
#   ifdef G3D_WIN32
        ...
        debugAssert(ret == 0);
        size_t toRead = (size_t)G3DLite::min(m_bufferLength, m_length - m_alreadyRead);
...
#   else
        ...
        debugAssert(ret == 0);
        size_t toRead = (size_t)G3DLite::min<int64>(m_bufferLength, m_length - m_alreadyRead);
        ret = fread(m_buffer, 1, toRead, file);
...
BinaryInput::BinaryInput(
    ...
    if (compressed) {
        ...
        m_length = G3DLite::readUInt32(data, m_swapBytes);
...
BinaryInput::BinaryInput(
    ...
    _internal::currentFilesUsed.insert(m_filename);
      ...
      /* Keefe: Removing dependency on libzip
    std::string zipfile;
...
void BinaryInput::decompress() {
	alwaysAssertM(false, "BinaryInput::decompress() not supported in G3DLite");
    // Decompress
    // Use the existing buffer as the source, allocate
    // a new buffer to use as the destination.
    /**
    int64 tempLength = m_length;
    m_length = G3DLite::readUInt32(m_buffer, m_swapBytes);
...
std::string BinaryInput::readStringEven() {
    ...
    if (hasMore() && (G3DLite::isOdd(x.length() + 1))) {
        skip(1);
    }
    return x;
}
