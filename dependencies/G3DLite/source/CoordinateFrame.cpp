...
CoordinateFrame::operator Any() const {
    ...
    a.append(x, y, z, yaw);
    if ( ! G3DLite::fuzzyEq(yaw, 0.0f) || ! G3DLite::fuzzyEq(pitch, 0.0f) || ! G3DLite::fuzzyEq(roll, 0.0f)) {
        a.append(yaw);
        if (! G3DLite::fuzzyEq(pitch, 0.0f) || ! G3DLite::fuzzyEq(roll, 0.0f)) {
            a.append(pitch);
            if (! G3DLite::fuzzyEq(roll, 0.0f)) {
                a.append(roll);
            }
        }
    }
...
void CoordinateFrame::getXYZYPRRadians(float& x, float& y, float& z, 
    ...
    if (abs(look.y) > 0.99f) {
        ...
        yaw   = G3DLite::pi() + atan2(look.x, look.z);
        pitch = asin(look.y);
        roll  = 0.0f;
...
bool CoordinateFrame::fuzzyEq(const CoordinateFrame& other) const {
    ...
    for (int c = 0; c < 3; ++c) {
        for (int r = 0; r < 3; ++r) {
            if (! G3DLite::fuzzyEq(other.rotation[r][c], rotation[r][c])) {
                return false;
            }
        }
        if (! G3DLite::fuzzyEq(translation[c], other.translation[c])) {
            return false;
        }
    }
