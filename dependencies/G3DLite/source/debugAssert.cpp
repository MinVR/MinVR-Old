...
namespace G3DLite { namespace _internal {
...
static void createErrorMessage(
    ...
    const char* newline = "\n";
		...
		if (lastErr != 0) {
	        le = G3DLite::format("Last Error (0x%08X): %s\r\n\r\n", lastErr, (LPCSTR)realLastErr);
		}
    ...
    // Build the message.
    outMessage =
        G3DLite::format("%s%s%sExpression: %s%s%s:%d%s%s%s", 
...
bool _handleDebugAssert_(
	...
	// G3DLite::We are not includeing prompt.h because it has windowing. Set result to always break
    //int result = G3D::prompt(dialogTitle.c_str(), dialogText.c_str(), (const char**)choices, 3, useGuiPrompt);
	int result = cBreak;
	std::cout<<"============================="<<std::endl<<std::endl<<dialogText<<std::endl;
...
bool _handleErrorCheck_(
	...
	// G3DLite::We are not includeing prompt.h because it has windowing. Set result to always break
    //int result = G3DLite::prompt("Error", m.c_str(), (const char**)choices, 1, useGuiPrompt);
	std::cout<<"============================="<<std::endl<<std::endl<<dialogText<<std::endl;
...
void setAssertionHook(AssertionHook hook) {
    G3DLite::_internal::_debugHook = hook;
}

AssertionHook assertionHook() {
    return 	G3DLite::_internal::_debugHook;
}

void setFailureHook(AssertionHook hook) {
    G3DLite::_internal::_failureHook = hook;
}

AssertionHook failureHook() {
    return G3DLite::_internal::_failureHook;
}


void setConsolePrintHook(ConsolePrintHook h) {
    G3DLite::_internal::_consolePrintHook = h;
}

ConsolePrintHook consolePrintHook() {
    return G3DLite::_internal::_consolePrintHook;
}
...
std::string __cdecl debugPrintf(const char* fmt ...) {
    ...
    std::string s = G3DLite::vformat(fmt, argList);
    va_end(argList);
