...
void AABox::split(const Vector3::Axis& axis, float location, AABox& low, AABox& high) const {
    ...
    float L = G3DLite::min(location, lo[axis]);
    float M = G3DLite::min(G3DLite::max(location, lo[axis]), hi[axis]);
    float H = G3DLite::max(location, hi[axis]);
...
bool AABox::culledBy(
    ...
    const bool finite = 
        (abs(lo.x) < G3DLite::finf()) &&
        (abs(hi.x) < G3DLite::finf()) &&
        (abs(lo.y) < G3DLite::finf()) &&
        (abs(hi.y) < G3DLite::finf()) &&
        (abs(lo.z) < G3DLite::finf()) &&
        (abs(hi.z) < G3DLite::finf());
